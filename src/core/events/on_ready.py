    ...
    async def on_ready(self):
        ...
        if not self.bot.initialised:
            ...
            self.bot.loaded_cogs = {
                cog: os.path.getmtime(module_to_file(cog)) for cog in get_cogs()
            }
        ...
        await self.bot.change_presence(
            activity=Activity(
                ...
                name="Je sais, tu sais, on sais. Mais quand?",
            )
        )

        self.bot.logger.info("LANA Initializated!")
