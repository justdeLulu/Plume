...
class MuzzleModule(Cog):
    ...
    async def on_message(self, message: nextcord.Message):
        ...
        # Is Muzzled
        if (
            nextcord.utils.get(message.guild.roles, id=self.muzzledRoleId)
            in message.author.roles
        ):
            count = await self.bot.prisma.talkmistakes.count(
                where={
                    "trigger": "MUZZLED",
                    "user": {
            ...
            await reportTalkMistake(
                prismaClient=self.bot.prisma,
            ...
            embed.add_field(
                name="Nombre d'erreurs commise:", value=f":warning: ||**{count}**||"
            )

            embed.add_field(
                name="Temps restant:", value=f":sparkling_heart: ||Owner decide.||"
            )

            embed.add_field(
                name="Message:", value=f":envelope: ||{message.content}||", inline=False
            )
