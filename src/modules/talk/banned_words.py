...
class BannedWords(Cog):
    ...
    async def initializeWords(self):
        ...
        self.bannedWords = await self.bot.prisma.bannedwords.find_many(
            where={
                "active": True,
            }
        )
        ...
        self.bot.logger.info(
            f"Loaded {len(self.bannedWords)} banned expressions successfully!"
        )
    ...
    @main.subcommand(name="list", description="List des Expressions Bannis.")
    ...
    async def add(
        ...
        name: str = SlashOption(
            name="name", description="Nom de l'Expression.", required=True
        ),
        trigger: str = SlashOption(
            name="trigger", description="Expression interdites", required=True
        ),
        ...
        description: str = SlashOption(
            name="description", description="Commentaires..", required=False
        ),
    ):
        ...
        embed.add_field(name="Nom", value=f":sparkling_heart: {name}")

        embed.add_field(name="Trigger", value=f"||{trigger}||")

        await interaction.followup.send(
            content=f"Trigger `{trigger}` ajouter! Reloading Banned Words...",
            embed=embed,
        )

        await self.initializeWords()
        await interaction.followup.send(
            content=f"Banned Words reloaded!", delete_after=5
        )

        pass

    @main.subcommand(
        name="toggle", description="Active/Desactive une Expression bannis."
    )
    async def toggle(self, interaction: Interaction):
        pass

    @main.subcommand(name="secret", description="Oui. Oui. Oui.")
        ...
        if message.author.id != self.bot.subject_id:
            ...
            for item in self.bannedWords:
                if item.trigger.lower() in words:
                    await self.bot.get_channel(self.bot.log_channel).send(
